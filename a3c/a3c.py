...
from SumoEnv.environ import SumoEnv
...
def actor_learner_thread(num, env, session, graph_ops, summary_ops, saver):
    ...
    env = SumoEnv(sumo_env=env, cross_num=RESIZED_WIDTH, cross_status=RESIZED_HEIGHT,
    ...
    while T < TMAX:
        ...
        while not (terminal or ((t - t_start) == t_max)):
            ...
            if probs_summary_t % 100 == 0:
                print
                "P, ", np.max(probs), "V ", session.run(v_network, feed_dict={s: [s_t]})[0][0]
...
def evaluation(session, graph_ops, saver):
    ...
    # Wrap env with AtariEnvironment helper class
    env = SumoEnv(sumo_env=monitor_env, cross_num=RESIZED_WIDTH, cross_status=RESIZED_HEIGHT,
                  agent_history_length=AGENT_HISTORY_LENGTH)
